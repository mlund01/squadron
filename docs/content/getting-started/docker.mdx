...
Both images ship with `python3` and `pip` so Python plugins (released or built from local `pyproject.toml` source) work out of the box. Python plugins must declare their dependencies in `pyproject.toml`; Squadron creates an isolated venv per plugin under `/config/.squadron/plugins/`.
