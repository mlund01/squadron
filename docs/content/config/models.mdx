...
| `reasoning_models` | list | no | Alias keys (or API names) to treat as reasoning-capable. Required for Ollama if you want native reasoning; useful elsewhere to pre-enable a newly released cloud model. |
...
Squadron supports native reasoning ("extended thinking" on Anthropic, `reasoning_effort` on OpenAI, `thinking_config` on Gemini). Agents and commanders enable it via the `reasoning` attribute (`"low"`, `"medium"`, or `"high"`); see [Agents → Reasoning](/config/agents#reasoning).

For cloud providers, Squadron knows which models support reasoning and silently enables it when requested. For Ollama (and any other provider where the model name doesn't match the cloud naming conventions), you must declare reasoning-capable models via `reasoning_models`:
...
model "ollama" {
  ...
  reasoning_models = ["deepseek_r1", "qwen3", "gpt_oss"]
}
```

Each entry is either an alias key (left side of `aliases`) or a raw API model name. Unknown entries error at config load.
