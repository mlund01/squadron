...
The tool takes a `message` (required) and an optional `channel` override — a channel **name** (e.g. `"#ops"`) or id — falling back to the gateway's configured default channel. It is one-way (the agent doesn't wait for a reply); for interactive prompts use `builtins.human.ask` instead.
