...
Variables are stored in an encrypted vault at `.squadron/vars.vault` and can be referenced in HCL configs using `vars.name`. The vault is initialized automatically by [squadron quickstart](/cli/quickstart) or `squadron engage --init`.
...
The vault is initialized automatically by [squadron quickstart](/cli/quickstart) or `squadron engage --init`.
