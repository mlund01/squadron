...
Capability is detected from the resolved model name — Squadron recognises Claude 4.x, OpenAI o3/o4/gpt-5, and Gemini 2.5+/3.x as reasoning-capable. Setting `reasoning` on an agent or commander whose model isn't recognised is a no-op and logs a warning at startup; the agent runs as if the attribute weren't set. Local-server providers (Ollama, vLLM, LiteLLM) currently fall outside the prefix detector and behave the same way: even if the underlying model supports reasoning, Squadron won't enable it automatically.
