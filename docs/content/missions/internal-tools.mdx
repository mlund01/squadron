...
```json
{
  "summary": "Analyzed the data and found 3 key trends...",
  "succeed": true
}
```
...
```json
{
  "summary": "Classified request as billing issue based on keyword analysis.",
...
For **cross-mission routes**, include `mission_inputs` using the destination input definitions in the routing prompt. Supply actual task results; this call starts the destination mission when successful. Values accept native JSON types or string-encoded values:

```json
{
  "summary": "Escalation required due to severity.",
...
{
  "succeed": false,
  "reason": "Unable to complete — external API is unreachable."
}
```
...
When a mission declares at least one storage slot — a top-level `memory "name"` referenced in `memories = [...]`, a mission-scoped `memory { }`, or `scratchpad = true` — every agent in that mission automatically gets these file tools:

| Tool | Description |
|------|-------------|
| `file_list` | List files and directories in a slot, with optional recursion and pagination |
| `file_read` | Read a file's text contents (optionally capped by lines or bytes) |
| `file_view` | View an image (PNG/JPEG/GIF/WebP), SVG, or PDF as visual/document content the model can perceive |
