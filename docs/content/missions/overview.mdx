...
| `memories` | list | Shared memory references, e.g. `[memories.data]` (see [Memory](/missions/folders)) |
| `memory` | block | Mission-scoped memory; `type = "persistent"` (default) registers as `"mission"`, `type = "ephemeral"` registers as `"run"`. At most one of each per mission. |
