...
The same rule applies to `context.path` and `plugin.source` — see [Plugins → Path Resolution](./plugins#path-resolution) and [Contexts → Path Resolution](../missions/contexts) for the block-attribute variants (those add per-HCL-file-dir anchoring for the relative forms, since blocks know which file declared them).
