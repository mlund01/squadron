...
caller supplies a path or a base64 upload; agents read the file through a
read-only `input.<name>` slot rather than via objective interpolation. File
inputs cannot have a `default` or be `protected`.
