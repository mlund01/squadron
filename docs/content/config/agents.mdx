...
agent "researcher" {
  ...
  role        = "Deep-research agent for hard problems"
  reasoning   = "high"
  tools       = [builtins.http.get]
}
```
...
| Provider | Built-in reasoning models | Reasoning events streamed? |
|----------|----------------------------|----------------------------|
...
| Ollama | None by default — use `reasoning_models` on the model block | **Yes** — reasoning summaries via the Responses API on Ollama 0.13.3+ |

**Models that don't support native reasoning silently ignore the `reasoning` attribute** — they produce direct answers with no reasoning step. There is no chain-of-thought fallback.
...
### Declaring reasoning on local or custom models

For Ollama, vLLM, and other OpenAI-compatible deployments where Squadron can't tell which model supports reasoning, declare reasoning-capable models on the `model` block via `reasoning_models`:
...
model "ollama" {
  ...
  reasoning_models = ["deepseek_r1", "qwen3"]   # llama3 omitted — no thinking
}
```

Entries can be either alias keys (the HCL-side name) or raw API model names. Unknown entries error at config load time. The list is additive — it can force-enable a model the built-in detector doesn't know about, but doesn't disable models that the detector already recognises.
