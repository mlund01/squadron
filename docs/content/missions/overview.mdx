...
exposed to agents as a **read-only, text-only slot** named `input.<name>`,
which they read with the `file_read` / `file_grep` / `file_search` tools (the
same tools used for [memory](/missions/folders) and [packets](/missions/packets)).
...
Either way the file is staged into an isolated per-run slot. v1 accepts UTF-8
text files only (binary content is rejected on read), and a single file is
capped at 10 MB. Files outside the project root must be supplied via the
base64 form.
