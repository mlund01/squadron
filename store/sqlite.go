...
func (s *SQLiteSessionStore) AppendStructuredMessage(sessionID, role, content string, parts []MessagePart, createdAt, completedAt time.Time) error {
	tx, err := s.db.Begin()
	if err != nil {
	...
	if err := insertPartsSQLite(tx, msgID, parts); err != nil {
		return err
	}
...
func insertPartsSQLite(tx *sql.Tx, msgID int64, parts []MessagePart) error {
	if len(parts) == 0 {
		return nil
	}
...
func (s *SQLiteSessionStore) GetStructuredMessages(sessionID string) ([]StructuredMessage, error) {
...
func scanMessagePart(
	...
	p := MessagePart{
		Type:                 pType,
		Text:                 text.String,
		ToolUseID:            tuID.String,
		ToolName:             tuName.String,
		ToolInputJSON:        tuInput.String,
		...
		EncryptedContent:     encContent.String,
		ProviderName:         provName.String,
		ProviderType:         provType.String,
		ProviderDataJSON:     provData.String,
	}
	if isErr.Valid {
		b := isErr.Bool
		p.IsError = &b
