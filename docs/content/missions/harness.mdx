...
task "classify" {
  router {
    route { target = tasks.refund;   condition = "Customer wants a refund" }
    route { target = tasks.escalate; condition = "Complaint is severe" }
    route { target = tasks.close;    condition = "Issue is resolved" }
  }
}
```
