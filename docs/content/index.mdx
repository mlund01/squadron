...
Squadron is a declarative framework for building and running AI agent workflows — entirely from configuration, not code.

Define LLM-powered agents, connect them to tools and plugins, and orchestrate multi-step missions where agents reason, act, and collaborate autonomously. Squadron handles the hard parts — agent orchestration, tool execution, state management, conditional branching, and crash recovery — so you describe *what* you want your agents to accomplish, not how to wire them together.

## Why Squadron

Most agentic frameworks require writing code to orchestrate multi-agent workflows — glue code, retry logic, state serialization, branching. Squadron takes a different approach: your entire agentic workflow — agents, their tools, how they collaborate, and what they produce — is defined as configuration.
...
Squadron separates *strategic thinking* from *tactical execution* using a two-tier agent architecture:

- **Commanders** are LLM-powered orchestrators — they break down each task into subtasks, delegate work to agents, interpret results, query upstream context, and make routing decisions. They reason about *what* needs to happen.
- **Agents** are LLM-powered executors — they use tools (plugins, APIs, shell, browser) in a ReAct loop to accomplish specific objectives. They reason about *how* to accomplish it.
- **Missions** wire it all together — tasks form a dependency graph with static sequencing, conditional branching, and fan-out patterns. Each task gets a commander that autonomously manages the agents assigned to it.

Different models can fill different roles. A fast model can handle routing while a capable model handles complex analysis. Agents and commanders can use different providers (Anthropic, OpenAI, Gemini) within the same mission.
...
- **[Schedules & triggers](/missions/schedules)** — run missions on cron schedules or via webhook triggers with concurrency controls
...
- **[MCP Tools](/config/mcp_tools)** — pull tools from any external Model Context Protocol server (npm, GitHub, HTTP, or bare stdio command) into your agents
- **[MCP Host](/config/mcp_host)** — expose Squadron's missions, runs, and configs as MCP tools to Claude Desktop, Claude Code, Cursor, and other AI assistants
