...
```bash
docker run -v ./config:/config -p 8080:8080 \
...
This creates `.squadron/` in the current directory and sets up an encrypted vault with a passphrase stored in your OS keychain.
