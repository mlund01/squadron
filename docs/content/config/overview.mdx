...
| `mcp "name"` | Pull tools from an external [MCP server](/config/mcp) (npm, github, http, or bare command) |
| `mcp_host` | Start a built-in [MCP host](/config/mcp#host-mcp_host--block) to expose Squadron to AI clients |
