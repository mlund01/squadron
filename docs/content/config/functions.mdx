...
```hcl
skill "web_scraping" {
  description  = "Load when you need to scrape web pages"
  instructions = load("./skills/web_scraping.md")
}

task "complex" {
  objective = load("./objectives/complex_task.md")
}
```

### Path Resolution

`load()` uses Squadron's project-root-anchored path rule. The process working directory is **never** consulted — you can run squadron from any CWD and `load("skills/foo.md")` resolves to the same file on disk.

| Form | Resolves to |
|------|-------------|
| `./foo.md`, `../foo.md`, bare `foo.md` | The **project root** — the directory passed to `squadron -c <dir>` (load doesn't have a per-callsite "HCL file dir", so all relative forms anchor to the project root) |
| `@/foo.md` | Explicitly the project root (same as bare for load(); useful for self-documenting calls) |
| `/foo.md`, absolute | **Rejected** with `absolute paths are not allowed` |

After resolution:

- Any `..` traversal that escapes the project root is rejected (`escapes the project root <path>`).
- A path that resolves to the project root itself is rejected (`resolves to the project root <path> itself`).

The same rule applies to `packet.path` and `plugin.source` — see [Plugins → Path Resolution](./plugins#path-resolution) and [Packets → Path Resolution](../missions/packets) for the block-attribute variants (those add per-HCL-file-dir anchoring for the relative forms, since blocks know which file declared them).
...
Defines a file handed to a mission at run time. **Mission inputs only.** The
caller supplies a path or a base64 upload; agents access the file through a
read-only `input.<name>` slot rather than via objective interpolation — reading
text with `file_read` or viewing images/SVG/PDF with `file_view`. File inputs
cannot have a `default` or be `protected`.

**Signature:** `file(description, required?)`
...
See [File Inputs](/missions/overview#file-inputs) for the supply forms and
runtime behavior.
...
| `protected` | bool | Mark as sensitive — masked in logs and UI (mission inputs only) |
...
See [Tools](/config/tools) for full tool configuration.
...
See [Missions](/missions/overview) for full mission configuration.
...
See [Tasks](/missions/tasks) for full task configuration.
...
See [Datasets](/missions/datasets) for full dataset configuration.
