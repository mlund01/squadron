...
// CLI; WsbridgeCallbackSource routes through command center for squadrons
// connected to a command center instance.
//
// Call order is: Prepare → SetState (before Present, once state is generated
// by the flow) → Present → Wait → Close. SetState exists because the OAuth
// state value is generated by mcp-go *after* Prepare returns the redirect
// URI, but routing-through-commander needs to register the flow under that
// state before the user's browser hits the IdP.
