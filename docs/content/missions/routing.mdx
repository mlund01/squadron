...
When the commander selects a mission route, it supplies `mission_inputs` using the destination's input names, types, descriptions, and required status shown in its routing prompt. Completed task outputs remain available to the commander, but neither `submit_output` nor the completion summary automatically populates the destination's inputs.

Input values accept native JSON types or string-encoded values, such as `false` or `"false"` for a boolean. Omit optional inputs to use their defaults; explicit `null` is rejected. A `file` input accepts a project path or a base64 envelope (`{"filename": "...", "content_base64": "..."}`) as a JSON object or string. Protected inputs are not offered to the commander — they always take their configured value. The tool reports missing, unknown, and invalid inputs together before completing the source task, so the commander can correct the payload without starting another mission. A successful call starts the destination mission.
