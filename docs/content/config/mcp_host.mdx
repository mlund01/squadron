...
> **Renamed in 2026-04.** This block was previously called `mcp { ... }` (singleton, no label). It was renamed to `mcp_host` to free up the `mcp "name" { ... }` form for consumer blocks. Update old configs by renaming the block — no other changes required.
