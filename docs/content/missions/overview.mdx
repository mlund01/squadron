...
| `memories` | list | Shared memory references, e.g. `[shared_memories.data]` (see [Memory](/missions/folders)). Old name: `folders` (still accepted). |
| `memory` | block | Persistent mission-scoped memory, registered as `"mission"`. Old name: `folder` (still accepted). |
| `run_memory` | block | Per-run ephemeral memory, registered as `"run"`. Old name: `run_folder` (still accepted). |
