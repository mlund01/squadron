...
Variable names — like all [block names](/config/overview#block-naming) — must use only lowercase letters, digits, and underscores, and must not start with a digit. `squadron verify` rejects anything else:

```hcl
variable "anthropic_api_key" { }  # Good
variable "anthropicApiKey" { }    # Error — uppercase
```
