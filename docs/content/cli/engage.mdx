...
Run `engage` from the root of your Squadron project directory. State (vault, PID file, logs, store) lives in `<project>/.squadron/`, so the directory you're in determines which project the daemon belongs to.
