...
Agents reach all three through the same `file_list`, `file_read`, `file_view`, `file_create`, `file_delete`, `file_search`, and `file_grep` tools, naming the slot on each call via the `slot` parameter. **Every slot is writable** — there is no read-only mode.
...
All file tools take a required `slot` parameter — that's the slot name (`"memory"`, `"scratchpad"`, or a shared memory's label):

| Tool | Purpose |
|------|---------|
| `file_list` | List files and directories, with optional recursion and pagination |
| `file_read` | Read a file's text (optionally capped by lines or bytes) |
| `file_view` | View an image, SVG, or PDF as visual/document content the model can perceive |
