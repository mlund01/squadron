...
A **packet** is a read-only bundle of reference files attached to a mission. Use it when you want agents to be able to read documentation, knowledge bases, examples, prompt libraries, or any other reference material that should never change during the run.
...
| Allowed file types | UTF-8 text (`file_read`) + images/SVG/PDF (`file_view`) | any |
...
The file tools apply the following policy for packet slots:

| Tool | Behavior in a packet slot |
|------|---------------------------|
| `file_list` | Lists files normally |
| `file_read` | Returns text content. Rejects files that look binary (NUL byte in the first 8 KB) — use `file_view` for images/SVG/PDF |
| `file_view` | Returns an image (PNG/JPEG/GIF/WebP), SVG (rasterized to PNG), or PDF as visual/document content the model can perceive |
...
Packets are designed for UTF-8 text: Markdown, source code, JSON/YAML, prose, transcripts, anything an LLM can read directly via `file_read`. Images (PNG/JPEG/GIF/WebP), SVGs, and PDFs are also supported — agents perceive them with `file_view` (SVGs are rasterized to PNG; PDFs are sent as native document blocks). `file_read` still rejects binary content up front so agents don't get a wall of garbled bytes.

Other binary payloads (audio, video, archives, compiled binaries) and non-UTF-8 text encodings (e.g. UTF-16) aren't supported by either tool. If you need to expose that kind of material, put it in a writable memory slot that a pre-processor can stage on demand.
