...
| `memory` | block | Mission-scoped persistent memory (slot `"memory"`). At most one per mission. |
| `scratchpad` | block | Mission-scoped ephemeral per-run scratchpad (slot `"scratchpad"`). At most one per mission. |
