...
`mission_completed`, `mission_failed`, or `mission_stopped` — to one or both of
two channels: the configured [gateway](/config/gateways) (Discord, Slack, …) and
the [command center](/config/command_center).
...
| `events` | list(string) | **yes** | — | Which terminal events fire. Values: `mission_completed`, `mission_failed`, `mission_stopped`, or `"all"` (every terminal event). |
...
- **`mission_stopped`** — the lifecycle event only.
...
- Each channel's `events` list is required and must contain only valid values
  (`mission_completed`, `mission_failed`, `mission_stopped`, `"all"`).
